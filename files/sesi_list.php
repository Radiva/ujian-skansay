                                ...
                                <div class="container">
                                    <div class="row">
                                        <div class="col-md-12" id="row" style="display:block; margin:20px 10px 20px 10px;">
                                            <p style="text-align: center; font-size: larger;"><b>Sesi Ujian</b></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card">
                                <div class="container">
                                    <div class="row">
                                        <div class="col-md-12" id="row" style="display:block; margin:20px 10px 20px 10px;">
                                            <center>
                                                <p style="text-align: center;"><b>Pilih Sesi Ujian</b></p>
                                                <?php
                                                $query = "SELECT * FROM sesi WHERE aktif = true ORDER BY id ASC";
                                                $result = mysqli_query($conn, $query);
                                                if (mysqli_num_rows($result) > 0) {
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                ?>
                                                        <a href="team_list.php?ruang=<?= $_GET['ruang']; ?>&sesi=<?= $row['id']; ?>"><button type="button" class="btn btn-success" style="margin: 10px 10px 10px 10px;"><?php echo $row['nama']; ?></button></a>
                                                <?php
                                                    }
                                                }
                                                ?>
